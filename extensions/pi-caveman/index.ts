...
const CONFIG_PATH = join(homedir(), ".pi", "agent", "extensions", "utils", "pi-caveman", "caveman.json");
...
async function saveConfig(config: CavemanConfig): Promise<void> {
	...
	saveConfigQueue = saveConfigQueue.then(async () => {
		await mkdir(join(homedir(), ".pi", "agent", "extensions", "utils", "pi-caveman"), { recursive: true });
...
export default function caveman(pi: ExtensionAPI) {
	...
	async function openConfig(ctx: ExtensionContext) {
		...
		await ctx.ui.custom((_tui, theme, _kb, done) => {
			...
			container.addChild(new Text(theme.fg("dim", " Saved to extensions/pi-caveman/caveman.json"), 0, 0));
